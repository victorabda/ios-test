...
class DownloadOperation : Operation {
    ...
    init(session: URLSession, downloadTaskURL: URL, completionHandler: ((URL?, URLResponse?, Error?, String?) -> Void)?) {
        super.init()
        ...
        task = session.downloadTask(with: downloadTaskURL, completionHandler: { [weak self] (localURL, response, error) in
            ...
            */
            if let completionHandler = completionHandler {
                ...
                completionHandler(localURL, response, error, self?.task.originalRequest?.url?.absoluteString)
            }
